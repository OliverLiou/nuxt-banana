...
```typescript
async function getItem(itemId: string): Promise<GalleryItem | null> {
  const { data, error } = await supabase
    .from('gallery_items')
    .select('*')
    .eq('id', itemId)
...
interface UpdateItemInput {
  id: string             // Required, target item UUID
  title: string          // Required, non-empty
...
**Consumer**: `useGalleryAdmin()` composable (called from edit form save flow, not inline table toggle)

> Note: The admin table USwitch is `disabled: true` (display only). Toggling isActive is performed via the edit form.
...
```typescript
async function toggleActive(itemId: string, isActive: boolean): Promise<void> {
...
- The admin table USwitch is `disabled: true` (display only). This function is called programmatically from the edit form save flow.
...
CREATE TABLE gallery_items (
  ...
  badges      JSONB DEFAULT '[]'::jsonb,  -- Nullable at DB level; application enforces ≥1 badge on form submission
  "isActive"  BOOLEAN DEFAULT true,
  created_at  TIMESTAMPTZ DEFAULT now()
);
```

> Note: `badges` is stored as a JSONB array of `{ label, color }` objects. The column is nullable at the DB level; the application UI enforces at least one badge on form submission.
> `isActive` uses camelCase as per the existing codebase convention.

---

## Auth Tables (Pre-existing — NOT created by this feature)

### DB-008: Read User Profile
...
| **Fields selected** | `roleId` |
| **Returns** | `{ roleId: string } | null` |
| **Called by** | `userStore.fetchRole()` |
| **When** | After successful login on `/login` page |
