...
- implement階段, 實作 nuxt-ui compoment時, 務必使用 nuxt-ui skill (use nuxt-ui)
- 當程式碼出現重複性時, 請將重複的程式碼抽離成可重用的 Composable 或元件, 以提升程式碼的可維護性與可讀性, 例: h(resolveComponent('UButton'), { ... }) => const UButton = resolveComponent('UButton'); h(UButton, { ... })
- implement階段完成後, 務必回頭檢查寫完的程式碼有無warning或error, 有的話請修正, 若是npm run dev後可以解決的問題, 可以先忽略